...
static int rule_connect_external_ip(sqlite3 *db, sqlite3_int64 *seen_id)
{
    ...
    const char *sql =
        ...
        "AND c.dst_ip NOT LIKE '10.%' "
        "AND c.dst_ip NOT LIKE '192.168.%' "
        "AND c.dst_ip NOT LIKE '172.16.%' "
        "AND c.dst_ip NOT LIKE '172.17.%' "
        "AND c.dst_ip NOT LIKE '172.18.%' "
        "AND c.dst_ip NOT LIKE '172.19.%' "
        "AND c.dst_ip NOT LIKE '172.2%.%' "
        "AND c.dst_ip NOT LIKE '172.30.%' "
        "AND c.dst_ip NOT LIKE '172.31.%') "
        "ORDER BY e.id ASC;";
    int rc = 0;
